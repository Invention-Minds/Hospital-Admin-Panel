...
@Component({
  selector: 'app-followup-estimation',
  templateUrl: './followup-estimation.component.html',
  styleUrl: './followup-estimation.component.css'
})
export class FollowupEstimationComponent {
  ...
  selectedEstimation:any = null; // Store the selected service
...
this.fetchPendingEstimations();
  ...
  fetchPendingEstimations(): void {
    this.isLoading = true
    this.estimationService.getFollowUpEstimation().subscribe({
      next: (estimation: any[]) => {
        console.log(estimation)
        this.pendingEstimations = estimation
  ...
  getLatestFollowUpRemark(estimation: any): string {
    if (!estimation.followUpDates || estimation.followUpDates.length === 0) {
      return 'No remarks';
    }
  
    const latest = estimation.followUpDates
