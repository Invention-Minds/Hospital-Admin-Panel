...
export class PatientDetailsComponent {
  ...
  constructor(private messageService: MessageService, private patientService: AppointmentConfirmService) { }
  patients: any[] = [];
    ...
    this.patientService.getAllPatients().subscribe({
      next: (patients: any[]) => {
        ...
        this.patients.sort((a, b) => {
          const dateA = new Date(a.prn!);
          const dateB = new Date(b.prn!);
          return dateB.getTime() - dateA.getTime();
        });

        // Store a copy for filtering
        this.filteredAppointments = [...this.patients];
        console.log('Confirmed appointments:', this.patients);
      },
      error: (error) => {
  ...
  onSearch(): void {
    ...
    this.filteredAppointments = this.patients.filter((service) => {
      let matches = true;
