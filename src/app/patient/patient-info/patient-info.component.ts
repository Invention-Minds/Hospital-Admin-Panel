...
import {
  assessmentDetailBlocks,
  assessmentTemplatedGroups,
  buildSavedVisitSummaryContent,
  eyeRecordPdfBlocks,
  eyeTable,
  eyeVaTable,
  hasDilatedAr,
...
export class PatientInfoComponent implements OnInit {
  ...
  selectedService: any[] = [];
  activeView: string = '';
  currentReportTitle = 'Common blood test investigation';
  @Input() patient: any;
  ...
  prescription: any[] = [];
  marqueeText: string = ''; // Initialize marqueeText
  historyNotes: any[] = [];
  ...
  constructor(private route: ActivatedRoute, private patientService: AppointmentConfirmService, private secondService: AppointmentConfirmService, private cdr: ChangeDetectorRef) { }
  @ViewChild('printSection') printSection!: ElementRef;
  ngOnInit(): void {
    console.log(this.patient)
    this.prn = this.patient.prn;
    if (this.prn) {
      this.fetchPatientData(this.prn);
    }
  }
  ...
  fetchPatientData(prn: string) {
    this.patientService.getDetailsByPRN(prn.toString()).subscribe((res: any) => {
      ...
      this.historyNotes = historyData;
      this.investigationOrders = investigationOrders;
      ...
      // Then label them in DESCENDING order (latest = highest label)
      ...
      // this.opdDates = appointments.map((d: any, i: number) => ({
      //   date: d.date,
      //   label: i + 1,
      //   notes: d
      // }));
      // this.opdDates.sort((a, b) => new Date(b.date).getTime() - new Date(a.date).getTime());
  ...
  toggleView(view: string, date: string, apptId?: number | null) {
    ...
    this.selectedService = this.serviceAppointments.filter(
      (appt: any) => appt.appointmentDate === date
    );
  ...
  private resolveVisitRecords(date: string, apptId?: number | null): void {
    ...
    const { assessment, eyeRecord } = resolveVisitRecords({
      assessments: this.opdAssessments,
      eyeRecords: this.ophthalmologyPrescriptions,
      date,
      appointmentId: id,
    });
    this.selectedAssessment = assessment;
    this.selectedEyeRecord = eyeRecord;
  }
  ...
  assessmentDetailBlocks(): { title: string; rows: { label: string; value: string }[] }[] {
    return assessmentDetailBlocks(this.selectedAssessment);
  }
  ...
  private async buildAssessmentStyleSummary(): Promise<any> {
    ...
    // Body shared with today-consultations' visit-summary print + WhatsApp.
    const content = buildSavedVisitSummaryContent({
      assessment: this.selectedAssessment,
      eyeRecord: this.selectedEyeRecord,
      prescriptions: this.selectedPrescription,
      investigationOrders: this.selectedInvestigationOrders,
      fallbackPatientName: this.patientData?.patientName,
    });
  ...
  // Eye record + template views — implementations live in the shared visit
  // summary builder so this screen, its print and today-consultations agree.

  /** Ophthalmology work-up for the selected visit as pdfMake blocks. */
  private eyeRecordPdfBlocks(): any[] {
    return eyeRecordPdfBlocks(this.selectedEyeRecord);
  }

  eyeTable(prefix: string): { label: string; right: string; left: string }[] {
    return eyeTable(this.selectedEyeRecord, prefix);
  }

  eyeVaTable(): { label: string; right: string; left: string }[] {
    return eyeVaTable(this.selectedEyeRecord);
  }

  get hasDilatedAr(): boolean {
    return hasDilatedAr(this.selectedEyeRecord);
  }

  assessmentTemplatedGroups(): { group: string; rows: { label: string; value: string }[] }[] {
    return assessmentTemplatedGroups(this.selectedAssessment);
  }
  ...
  toggleVisit(index: number) {
    this.expandedVisitIndex = this.expandedVisitIndex === index ? null : index;
  }
  printPrescription() {
    const contents = this.printSection.nativeElement.innerHTML;
  ...
  printFullVisit(date: string) {
    this.selectedVisit = this.doctorNotes.find(d => d.date === date) || {};
    this.selectedVitals = this.bloodGroupAppointments.find((appt: any) => appt.date === date);
    this.selectedService = this.serviceAppointments.filter((appt: any) => appt.appointmentDate === date);
    this.selectedPrescription = this.prescription.filter((appt: any) => appt.prescribedDate === date);

    // Trigger change detection and allow DOM to update
    this.cdr.detectChanges();

    // Delay to ensure DOM + images render
    setTimeout(() => {
      const printContents = document.getElementById('printFullSection')?.innerHTML;
      ...
      if (printWindow && printContents) {
        printWindow.document.write(`
          <html>
            <head>
              <title>OPD Visit Summary - ${date}</title>
              <style>
              @media print {
                          .page-break {
...
.vital-column {
  ...
  async printVisitSummary(date: any, apptId?: number | null) {
    ...
    // The summary builder reads selectedInvestigationOrders. It was only set by
    // toggleView, so printing without opening the visit first dropped the
    // investigations (or printed the last-opened visit's orders).
    this.selectedInvestigationOrders = this.investigationOrders.filter((appt: any) => appt.date === date);
  ...
  generatePdfWithPatient(patient: any, vitals: any, visit: any,filteredInvestigationOrders:any[], history:any, prescriptions: any[], transfusion: any[]) {
